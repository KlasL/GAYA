...
!-- EGNA DEKLARATIONER
       ...
       DATA	WriteTest/.false./,ModelII/.true./,FirstWrite/.true./
...
!-- non-factor info
     	if( WriteTest .AND. IFIX(StandID) == 9)then
          do iiper=iper1,iper,iper2
...
!-- Write data for AIMMS and for further analysis
        ...
        'm3Harv,SCost,HCost'
            ...
            if(FirstWrite)then
                write(Cout,'(99a)')	&
        'Plots,Progs,Periods,Version,NRatg,Treatm,', &
        'Age,m3Vol,Hgv,Dia,Stems,',	&
        'TSbiom,TScoar,TSfine,', &
        ( 'Age'//Species(i)//',',i=1,MXSPECI ), &
        ( 'Vol'//Species(i)//',',i=1,MXSPECI ), &
        ( 'Hei'//Species(i)//',',i=1,MXSPECI ), &
        ( 'Diam'//Species(i)//',',i=1,MXSPECI ), &
        ( 'Stem'//Species(i)//',',i=1,MXSPECI )
                                k=0
                do i = 1,len(TRIM(Cout))
                        if(Cout(i:i) /=  ' ')then
                            k = k + 1
                ...
                nratg(iiper),CTRT(IXATG(nratg(iiper))),	& 
                 ...
                 TScoar(iiper), &
                 TSfine(iiper), &
                 nint(ARTin(BHAs,1:MXSPECI,iiper) ),	&
                 nint(ARTin(Vs,1:MXSPECI,iiper) ),	&
                 nint(10.*ARTin(Hs,1:MXSPECI,iiper)), &
                 ( nint(10.*G3DIA(ARTin(Gs,i,iiper),ARTin(Ns,i,iiper))), i=1,MXSPECI ), &
                 nint(ARTin(Ns,1:MXSPECI,iiper) )
                k=0
                do i = 1,len(TRIM(Cout))
                        if(Cout(i:i) /=  ' ')then
                            k = k + 1
            ...
            ! factor info (excl. cost)
                if(FirstWrite)then
                 ...
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'PinTim',IFIX(StandID),jNr,iiper,restim(Pine,iiper)
  			            if( restim(Spruce,iiper) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'SprTim', IFIX(StandID),jNr, iiper,restim(Spruce,iiper)
  			            if( sum(restim(3:MXSPECI,iiper)) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'BerTim', IFIX(StandID),jNr, iiper,sum(restim(3:MXSPECI,iiper))	! Assumes that only birch could be timber
  			            if( resmav(Pine,iiper)+ resmav(Contorta,iiper) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'PinPuw', IFIX(StandID),jNr, iiper,(resmav(Pine,iiper) + resmav(Contorta,iiper))
  			            if( resmav(Spruce,iiper) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'SprPuw', IFIX(StandID),jNr, iiper,resmav(Spruce,iiper)
  			            if( sum(resmav(3:MXSPECI,iiper) - resmav(Contorta,iiper)) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'Decids', IFIX(StandID),jNr, iiper,(sum(resmav(3:MXSPECI,iiper)) - resmav(Contorta,iiper) ) 
			            if( ResStm(iiper) > 1. )	&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'ResStm', IFIX(StandID),jNr, iiper, ResStm(iiper)
  			            if( ResRot(iiper) > 1. )&
                 write(70,'(2x,a,2x,i6,1x,a,i4,f11.1)')	&
                 'ResRot', IFIX(StandID),jNr, iiper, ResRot(iiper) 
