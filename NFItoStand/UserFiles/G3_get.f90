...
!-- EGNA DEKLARATIONER
INTEGER, PARAMETER :: Nstands=10000		! Max no. of stands
      ...
      real :: StdFarwDist
      data StdFarwDist/0./ ! Standard distance to road for running unique NFI plots (0 to make driving time 0)
! For genmethod 3    
       integer      :: NoNFI,NFInr(Nstands)
       character    :: NFItoUse(Nstands)*12
...
!-- NOLLSTŽLLNING ------------------------------------------------------
    ...
    if(Start)then
        ...
        NoNFI = 1
 		do while(Start)
            read(68,*,end=5)NFItoUse(NoNFI)
            NoNFI = NoNFI + 1
        end do
5       NoNFI = NoNFI - 1
        if(NoNFI == 0)stop('NoNFI == 0')
...
!           16= ljung  17= odon/skvattram  18= rosling/tranb„r
       ...
       FIX(mToRoad) = StdFarwDist ! Use standard distance to road for running unique NFI plots
! End read
!-- Select -------------------------------------
      if(Rnd > RndLim)then
        	ierr = 1
      ...
      if(.NOT.ANY(NFItoUse(1:NoNFI) == CFIX(StandID)))then
        	ierr = 1
            RETURN
      endif
...
! Recalculate domain to be used to limit new forest options
        IFIX(Domain) = 2
        if(BESTin(PerFF,1) < 1.)IFIX(Domain) = MAXLOC(ARTin(Gs,:,1),1)
...
! Stems ha-1
        ...
        FNART(Ns,OtherBrl,NewFor),FNART(Ns,Larch,NewFor), &
! Basal area (m2)
        ...
        FNART(Gs,OtherBrl,NewFor),FNART(Gs,Larch,NewFor), &
! Height (m)
        ...
        FNART(Hs,OtherBrl,NewFor),FNART(Hs,Larch,NewFor), &
! BHA (y)
        ...
        FNART(BHAs,OtherBrl,NewFor),FNART(BHAs,Larch,NewFor)
