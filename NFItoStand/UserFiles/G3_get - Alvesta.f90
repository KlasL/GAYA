...
!            NFI_mToRoad(NoNFI), &		! stand/plot metres from road
            ...
            NFI_DeadWood(NoNFI), &		! Initial dead wood
            NFI_DominantSp(NoNFI), &    ! Dominant species
            NFI_RegenerationSp(NoNFI), &    ! Regeneration species
...
!    read(cin2(18),*)   St_Domain
    ...
    IFIX(CutClass) = NFI_Hkl(j)
    IFIX(DominSp) = NFI_DominantSp(j)
    IFIX(RegenSp) = NFI_RegenerationSp(j)
...
subroutine simple_dictionary
    ...
    type(KeyValue), allocatable :: dict(:)
    integer :: i, n

    ! Add entries
    n = 3
    allocate(dict(n))
    dict(1) = KeyValue('apple', 10)
    dict(2) = KeyValue('banana', 20)
    dict(3) = KeyValue('cherry', 30)

    ! Lookup example
    do i = 1, n
        if (dict(i)%key == 'banana') then
            print *, 'Value for banana:', dict(i)%value
        end if
    end do
end subroutine simple_dictionary