...
! Qualities and transport (from 
        ...
        data    TransferToPulpwood/0.,0.,0.6,9*1./
! Biomass
		REAL ::	StemToTScoar
...
30          CONTINUE
            ! Costs
!!!!!---
! Justering av gallringskostnad mot https://www.skogskunskap.se/aga-skog/priser--kostnader/kostnader-for-gallring/
! Reducering till 60% av beräknad skördarkostnad; + 50 kr på beräknad terrängtransportkostnad FF
            
            I = MAXLOC(CUT(1,:,IPER),1)
            VTOT=sum(restim(:,IPER)) + sum(resmav(:,IPER))
            if(IATYP == FF)then
				CostHarv(IPER) = HarvTimeFi(1,I,2,sum(CUT(1,:,IPER))/sum(CUT(2,:,IPER)))	&
     				* VTOT * HarvHcostFF
            else
				CostHarv(IPER) = HarvTimeFi(2,I,2,sum(CUT(1,:,IPER))/sum(CUT(2,:,IPER)))	&
     				* VTOT * HarvHcostTh * 0.6
			endif
            CostForw(IPER) = ForwTimeFi(1,1,1,VTOT,FIX(mToRoad)) * VTOT * ForwHcostFF
            if(IATYP == Th)CostForw(IPER) = CostForw(IPER) + 50. * VTOT
            CostGROT(IPER) = GROTtime(ODT(IPER),FIX(mToRoad),53.,10.,0.75) * ForwHcostTh
