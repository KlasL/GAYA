...
!-- OPEN COMMAND FILE -------------------------------------------------
fname = 'ComGAYA.txt'
OPEN(nrfil(8),FILE=fname,STATUS='old',ERR=4)
go to 10
4   WRITE(*,'(a)')'fname= '
READ(*,'(a)')fname
...
icont = MAX(icont,lrsk)
135 iatg(icont)=iatg(icont)+1
IF(iatg(icont)<= natg .AND. icont >= lrsk .AND. ixatg(iatg(icont)) /= 0)GO TO 130
